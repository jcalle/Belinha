...
void TEulerDiffusivity::JacobFlux(TPZVec<REAL> &U,TPZFMatrix<STATE> &Ajacob,TPZFMatrix<STATE> &Bjacob) {
  if(U[0] > -1.e-6 && U[0] < 1.e-6) {
...
void TEulerDiffusivity::ValJacobFlux(TPZVec<REAL> &U,TPZFMatrix<STATE> &valjacob,TPZVec<REAL> &normal) {
  valjacob.Zero();
  REAL rho = U[0];
  REAL vx, vy, cval, cval2, u, kt, st, aa, bb;
  REAL gM1 = fGamma-1.;
  vx = U[1]/rho;
  vy = U[2]/rho;
  cval2 = (fGamma*Pressure(U))/rho;
  if(cval2<0) {
	  std::cout << "TEulerLaw2D::ValJacobFlux. velocidade do som negativa.\n";
    return;
  }
  cval = sqrt(cval2);
...
void TEulerDiffusivity::InverseJacob(TPZFMatrix<STATE> &jac) {
  ...
  if(det > -1.e-6 && det < 1.e-6) {
	  std::cout << "TEulerLaw2D::InverseJacob. Determinante zero, matriz singular.\n";
	  exit(1);
  }
...
void TEulerDiffusivity::InvJacob2d(TPZFMatrix<STATE> &axes,TPZFMatrix<STATE> &jacinv) {
...
void TEulerDiffusivity::MatrixDiff(TPZVec<REAL> &sol,TPZFMatrix<REAL> &axes, TPZFMatrix<STATE> &jacinv,TPZFMatrix<STATE>
...
void TEulerDiffusivity::JacobFlux(TPZVec<REAL> &U,TPZFMatrix<STATE> &jacob,TPZVec<REAL>
&normal) {
...
int TEulerDiffusivity::main() {
  ...
  eul.Flux(U,flux);
	...
	TPZFMatrix<STATE> jacinv(2, 2, 0.), ATA(4, 4), ATB(4, 4), BTA(4, 4), BTB(4, 4);
