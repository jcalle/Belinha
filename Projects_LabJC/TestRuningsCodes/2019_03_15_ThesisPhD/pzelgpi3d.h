//HEADER FILE FOR CLASS ELEMPI3D
...
/** Header file for the three dimensional pyramid element class*/

class TPZGeoElPi3d : public TPZGeoElRefLess<pzgeom::TPZGeoPyramid> {
  /** pointer to function which creates a computational element*/
  static TPZCompEl *(*fp)(TPZGeoElPi3d *geoel,TPZCompMesh &mesh, int64_t &index);

 protected:
  /**Indices of the nodes which compose the element*/
	 int64_t fNodeIndexes[5];
  /**Pointers to the neighbouring elements with corresponding sides*/
  TPZGeoElSide fNeighbours[19];
 ...
 public:
  ...
  TPZGeoElPi3d(int id,TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);
  TPZGeoElPi3d(TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);

  TPZGeoElPi3d *CreateGeoEl(TPZVec<int64_t> &nodeindices,int matind,TPZGeoMesh &mesh);
  ...
  static void SetCreateFunction(TPZCompEl *(*f)(TPZGeoElPi3d *el,TPZCompMesh &mesh, int64_t &index));
  ...
  int NSides() { return 19;}//uma para cada conectividade

  /**Return the number of nodes for a particular side*/
  int NSideNodes(int side);

  int NCornerNodes() { return 5;}
...
inline TPZCompEl *TPZGeoElPi3d::CreateCompEl(TPZCompMesh &mesh, int64_t &index){
  return fp(this,mesh,index);
}

inline void TPZGeoElPi3d::SetCreateFunction(TPZCompEl *(*f)(TPZGeoElPi3d *el,TPZCompMesh &mesh, int64_t &index)){
  fp = f;
}
