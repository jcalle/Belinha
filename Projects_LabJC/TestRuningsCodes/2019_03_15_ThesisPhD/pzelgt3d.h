//HEADER FILE FOR CLASS ELEMGT3D
...
/** Header file for the three dimensional tetrahedra element class*/

class TPZGeoElT3d : public TPZGeoElRefLess<pzgeom::TPZGeoTetrahedra> {

  /** pointer to function which creates a computational element*/
  static TPZCompEl *(*fp)(TPZGeoElT3d *geoel,TPZCompMesh &mesh, int64_t &index);
 ...
 public:
  ...
  TPZGeoElT3d(int id,TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);
  TPZGeoElT3d(TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);

  TPZGeoElT3d *CreateGeoEl(TPZVec<int64_t> &nodeindices,int matind,TPZGeoMesh &mesh);
  /**Destructor*/
  ~TPZGeoElT3d();

  TPZCompEl *CreateCompEl(TPZCompMesh &mesh, int64_t &index);

  static void SetCreateFunction(TPZCompEl *(*f)(TPZGeoElT3d *el,TPZCompMesh &mesh, int64_t &index));
  ...
  int NSides() { return 15;}//uma para cada conectividade
...
inline TPZCompEl *TPZGeoElT3d::CreateCompEl(TPZCompMesh &mesh, int64_t &index){
  return fp(this,mesh,index);
}

inline void TPZGeoElT3d::SetCreateFunction(TPZCompEl *(*f)(TPZGeoElT3d *el,TPZCompMesh &mesh, int64_t &index)){
  fp = f;
}
