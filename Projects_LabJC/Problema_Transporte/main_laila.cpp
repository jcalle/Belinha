...
const int bc3 = -4;
REAL Lx = 2., Ly = 1.;
//to create geometrical and computacional meshes
...
void CreatingGeoMesh(TPZGeoMesh*Gmesh3);
//Riemann Problem
bool triang = false;
...
void SetBCIdForElements(TPZGeoMesh*gmesh, int BCId);
int main(int argc, char *argv[])
{
	...
	//TPZGeoMesh *gmesh = GMesh3(Lx,Ly);
		...
		GeoGridFile = "Um_quarto_malha_simples.dump";
		gmesh = myreader.GeometricGIDMesh(GeoGridFile);
		// Inserting material id
		SetMaterialIdForElements(gmesh, matId);
		SetBCIdForElements(gmesh, BCId);
	...
	//Relativo a malha computacional
	TPZMatConvectionProblem *material;

	TPZCompMesh *cmesh = MalhaComp(gmesh, p, material);
	CreatInterface(cmesh);
	TPZCompMesh *MalhaComp2(TPZGeoMesh * gmesh, int p/*,TPZMatConvectionProblem * &material*/);
