//HEADER FILE FOR CLASS ELEMPR3D
...
/** Header file for the three dimensional prism element class*/

class TPZGeoElPr3d : public TPZGeoElRefLess<pzgeom::TPZGeoPrism>
{
  /** pointer to function which creates a computational element*/
  static TPZCompEl *(*fp)(TPZGeoElPr3d *geoel,TPZCompMesh &mesh, int64_t &index);

 protected:
  /**Indices of the nodes which compose the element*/
	 int64_t fNodeIndexes[6];
  /**Pointers to the neighbouring elements with corresponding sides*/
  TPZGeoElSide fNeighbours[21];
  /**Pointers to subelements. If has less than 8 sub-elements the pointer is NULL*/
  TPZGeoEl *fSubEl[8];
 ...
 public:
  ...
  TPZGeoElPr3d(int id,TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);
  TPZGeoElPr3d(TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);

  TPZGeoElPr3d *CreateGeoEl(TPZVec<int64_t> &nodeindices,int matind,TPZGeoMesh &mesh);
  ...
  static void SetCreateFunction(TPZCompEl *(*f)(TPZGeoElPr3d *el,TPZCompMesh &mesh, int64_t &index));
  ...
  int NSides() { return 21;}//uma para cada conectividade

  /**Return the number of nodes for a particular side*/
  int NSideNodes(int side);

  int NCornerNodes() { return 6;}
...
inline TPZCompEl *TPZGeoElPr3d::CreateCompEl(TPZCompMesh &mesh, int64_t &index){
  return fp(this,mesh,index);
}

inline void TPZGeoElPr3d::SetCreateFunction(TPZCompEl *(*f)(TPZGeoElPr3d *el,TPZCompMesh &mesh, int64_t &index)){
  fp = f;
}
