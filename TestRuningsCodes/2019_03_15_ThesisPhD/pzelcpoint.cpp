...
TPZCompElPoint::TPZCompElPoint(TPZCompMesh &mesh,TPZGeoElPoint *ref, int64_t &index) :
	TPZIntelGen<pzshape::TPZShapePoint>(mesh,ref,index), fIntRule(1) {
  // Jorge 19/5/99
...
TPZCompElPoint::TPZCompElPoint(TPZCompMesh &mesh,TPZGeoElPoint *ref, int64_t &index,int /*noconnects*/) :
	TPZIntelGen<pzshape::TPZShapePoint>(mesh,ref,index), fIntRule(1) {
...
int TPZCompElPoint::NConnectShapeF(int side) {
	  if(side==0)  return 1;
     PZError << "TPZCompElPoint::NConnectShapeF, bad parameter iconnect " << side << std::endl;
     return 0;
}
...
int TPZCompElPoint::SideConnectLocId(int c, int side) {
    ...
    PZError << "TPZCompElPoint::SideConnectLocId, connect = " << c << std::endl;
    return -1;
}
...
void TPZCompElPoint::SetPreferredSideOrder(int side, int /*order*/) {
  PZError << "TPZCompElPoint::SetPreferredSideOrder. Bad paramenter side.\n";
	if(side != 0) return;
}


void TPZCompElPoint::SetSideOrder(int /*side*/, int /*order*/) {
  PZError << "TPZCompElPoint::SetSideOrder. Bad paramenter side.\n";
}
...
void TPZCompElPoint::CornerShape(TPZVec<REAL> &/*pt*/,TPZFMatrix<STATE> &phi,TPZFMatrix<STATE> &dphi) {
  phi.Redim(1,1);
  phi(0,0) = 1.;
  dphi.Redim(0,1);
}

void TPZCompElPoint::SideShapeFunction(int side,TPZVec<REAL> &,TPZFMatrix<STATE> &phi,TPZFMatrix<STATE> &dphi) {  //    id ???
  if(side!=0) {
...
void TPZCompElPoint::Shape(TPZVec<REAL> &,TPZFMatrix<STATE> &phi,TPZFMatrix<STATE> &dphi) {
  phi.Redim(1,1);
  dphi.Redim(0,1);
  phi(0,0) = 1.;
}
