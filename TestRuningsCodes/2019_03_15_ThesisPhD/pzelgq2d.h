...
/** Header file for the two dimensional rectangular element class*/

class TPZGeoElQ2d : public TPZGeoElRefLess<pzgeom::TPZGeoQuad> {
  /** pointer to function which creates a computational element*/
  static TPZCompEl *(*fp)(TPZGeoElQ2d *geoel,TPZCompMesh &mesh, int64_t &index);
  
 protected:
  /**Indices of the nodes which compose the element*/
  int64_t fNodeIndexes[4];
  /**Pointers to the neighbouring elements with corresponding sides*/
  TPZGeoElSide fNeighbours[9];
 ...
 public:
  ...
  TPZGeoElQ2d(int id,TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);
  TPZGeoElQ2d(TPZVec<int64_t> &nodeindexes,int matind,TPZGeoMesh &mesh);
  
virtual  TPZGeoElQ2d *CreateGeoEl(TPZVec<int64_t> &nodeindices,int matind,TPZGeoMesh &mesh);
  /**Destructor*/
  ~TPZGeoElQ2d();
  
  TPZCompEl *CreateCompEl(TPZCompMesh &mesh, int64_t &index);

  static void SetCreateFunction(TPZCompEl *(*f)(TPZGeoElQ2d *el,TPZCompMesh &mesh, int64_t &index));
  ...
  int NSides() { return 9;}//uma para cada conectividade

  /**Return the number of nodes for a particular side*/
  int NSideNodes(int side);

  int NCornerNodes() { return 4;}
  ...
  **/
  ...
  /*para checar as estructuras estaticas implementadas*/
...
inline TPZCompEl *TPZGeoElQ2d::CreateCompEl(TPZCompMesh &mesh, int64_t &index){
  return fp(this,mesh,index);
}

inline void TPZGeoElQ2d::SetCreateFunction(TPZCompEl *(*f)(TPZGeoElQ2d *el,TPZCompMesh &mesh, int64_t &index)){
  fp = f;
}
